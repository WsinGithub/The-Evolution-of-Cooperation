...
turtles-own [
  ...
  strategy
  partners
  play-rec   ; [ [ (turtle 0) "C" ] [ (turtle 1) "D" ] ...
]
...
to setup
  clear-all
  init-strategy-list
  set gain-list (list (list "CC" gain-CC gain-CC) (list "CD" gain-CD gain-DC) (list "DC" gain-DC gain-CD) (list "DD" gain-DD gain-DD))
  ask patches [ sprout-1-turtle "random"]
  update-label-by-money
  reset-ticks
end

to sprout-1-turtle [option]
  sprout 1 [
    set money 100
    set shape "square"
...
to find-partners
  set partners turtles-on neighbors4
end
...
to change-strategy
  if money < 50 [set strategy (choose-strategy "neighbor-best")]
end
...
to play-with-partners
  while [any? partners][
    let A self
    let B one-of partners
    set partners partners with [self != B]
    ask B [set partners partners with [self != A]]
    let action-A [(runresult strategy B)] of A
...
to write-play-rec [rec]
  set play-rec lput rec play-rec
  while [length play-rec > memory-length * 4 ][set play-rec but-first play-rec]
end

to-report choose-strategy [option]
  if option = "random" [report (one-of strategy-list)]
  if option = "global-best" [report [strategy] of max-one-of turtles [money]]
  if option = "neighbor-best" [report [strategy] of max-one-of turtles-on neighbors [money]]
end
...
to update-label-by-money
  ask turtles [set label money]
end
@#$#@#$#@
GRAPHICS-WINDOW
267
